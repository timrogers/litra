...
import { Device, findDevices } from '../driver';
...
const getDeviceBySerialNumber = (
  ...
  devices.find((device) => device.serialNumber === requestedSerialNumber);
 ...
 */
export const parseIntOption = (value: string): number => {
 ...
 */
export const getDeviceForCLI = (serialNumber: string | undefined): Device => {
  ...
  if (device) {
    if (devices.length > 1 && !serialNumber) {
      console.warn(
        `⚠️ You have multiple devices connected, but you haven't specified the serial number of the device you want to target using \`-s\`. Using first identified device with serial number ${device.serialNumber}. This device is not guaranteed to be the same every time you run this command.`,
      );
    }
